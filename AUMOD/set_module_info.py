...
# 인식모듈 게시글의 정보를 수정하는 함수
# user_key : API 를 호출하는 사용자의 키
# module_index : 수정하려는 인식모듈 게시글의 인덱스
# module_name : 수정하려는 인식모듈 게시글의 제목
# version : 수정하려는 인식모듈 게시글의 버전
# memo : 수정하려는 인식모듈 게시글의 내용
def set_module_info(user_key, module_index, module_name, version, memo):
    ...
    input_list = [user_key, module_index, module_name, version, memo]
    ...
    # 현재 접속 중인 사용자일 경우
        ...
        try:
            # 작성자를 알기 위해 조회하는 것 이므로 조회수는 증가하지 않음
            count_click = "N"
            ...
            # 작성자 동일 또는 권한 존재 중 하나라도 값이 Y 라면 인식모듈 수정 가능
            if id_equal_flag == "Y" or auth_exist_flag == "Y":

                # 파일 관련 정보는 원래 입력되어있던 값 그대로 입력
                ...
                file_size = result_module_info[0]['FILE_SIZE']
                file_exist_flag = result_module_info[0]['FILE_EXIST_FLAG']
                # 모듈 수정하는 프로시저 호출
                cursor.callproc('SP_KEYM_SET_MODULE_INFO', [module_index, module_name, file_name_ori, file_name,
                                                            file_path, file_size, version, memo, file_exist_flag])
                ...
                log_status = "REQUEST"
                output = data
                message = "set_module_info success"
                success_flag = "Y"
                ...
                # 작성자가 아니거나 인식모듈 수정 권한이 존재하지 않는 경우
                data = []
                result = "fail"
                status = "402"
